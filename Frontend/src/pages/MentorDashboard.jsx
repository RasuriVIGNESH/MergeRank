...
import { Users, AlertTriangle, TrendingUp, Search, Filter, MoreVertical, ChevronRight } from 'lucide-react';
...
export function MentorDashboard() {
  ...
  const atRiskStudents = data.students.filter((s) => s.placementReadiness < 60).length;
  const placementReady = data.students.filter((s) => s.placementReadiness >= 75).length;
    ...
    <Layout role="mentor">
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <StatCard title="Placement Ready" value={placementReady} icon={TrendingUp} color="text-emerald-600" bg="bg-emerald-50" />
          <StatCard title="At Risk" value={atRiskStudents} icon={AlertTriangle} color="text-rose-600" bg="bg-rose-50" />
        </div>
                      ...
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-2">
                          <div className="w-16 h-2 bg-slate-100 rounded-full overflow-hidden">
                            <div
                              className={`h-full rounded-full ${readiness >= 75 ? 'bg-emerald-500' : readiness >= 60 ? 'bg-amber-500' : 'bg-rose-500'}`}
